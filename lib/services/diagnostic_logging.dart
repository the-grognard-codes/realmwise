...
class DiagnosticSanitizer {
  ...
  static final _secret = RegExp(r'(bearer\s+)?[A-Za-z0-9_\-]{16,}', caseSensitive: false);
  static final _sensitiveValue = RegExp(
    r'([A-Za-z]:\\|/Users/|/home/|https?://|\b(?:\d{1,3}\.){3}\d{1,3}\b|\b[^\s@]+@[^\s@]+\.[^\s@]+\b)',
    caseSensitive: false,
  );
  static final _allowed = <String>{
    'provider', 'operation', 'outcome', 'status', 'statusFamily',
    'elapsedMs', 'retryCount', 'errorClass', 'platform', 'feature',
    'count', 'schemaVersion',
  };

  static Map<String, Object?> fields(Map<String, Object?> input) =>
      Map<String, Object?>.fromEntries(input.entries
  ...
  static Object? value(Object? input) {
    ...
    return input.toString().replaceAll(_secret, '[REDACTED]');
  }

  static String exception(Object error, [StackTrace? stack]) {
    final message = value(error.toString()) as String;
...
class DiagnosticEvent {
  const DiagnosticEvent({required this.timestamp, required this.severity,
    required this.name, required this.fields, required this.sessionId});
  ...
  Map<String, Object?> toJson() => {
    ...
    'severity': severity.name, 'event': name, 'fields': fields,
    'session': sessionId,
  };
}

class DiagnosticLogger {
  DiagnosticLogger({Directory? directory, String? sessionId, this.maxBytes = 5 * 1024 * 1024})
  ...
  static String _newSession() => '${DateTime.now().toUtc().microsecondsSinceEpoch}-${Random.secure().nextInt(1 << 32).toRadixString(16)}';

  Future<void> initialize() async {
    _directory ??= Directory(p.join((await getApplicationSupportDirectory()).path, 'diagnostics'));
    await _directory!.create(recursive: true);
    await _reconcile();
  }
  Directory get directory => _directory!;
  bool _retained(DiagnosticSeverity s) => s.index >= DiagnosticSeverity.warning.index || debugLogging;

  Future<void> configure({required bool optionsEnabled, required bool debugEnabled}) async {
  ...
  Future<void> log(DiagnosticSeverity severity, String event, [Map<String, Object?> fields = const {}]) async {
    if (!_retained(severity)) return;
    try {
      await initialize();
      final item = DiagnosticEvent(timestamp: DateTime.now().toUtc(), severity: severity,
          name: DiagnosticSanitizer.eventName(event), fields: DiagnosticSanitizer.fields(fields), sessionId: sessionId);
      ...
      await _enqueue<void>(() async {
        ...
        if (await file.exists() && (await file.length()) + line.length > maxBytes) {
          final rotated = File(p.join(directory.path,
              'diagnostic-${DateTime.now().toUtc().microsecondsSinceEpoch}.log'));
          try { await file.rename(rotated.path); } on Object { /* best effort */ }
        }
        await file.writeAsString(line, mode: FileMode.append, flush: true);
        await _reconcile();
      });
    } on Object { /* Logging is deliberately best effort. */ }
  }

  Future<List<File>> files() async {
    if (_directory == null) return const [];
    try { return (await directory.list().where((e) => e is File).toList()).cast<File>(); }
    on Object { return const []; }
  }
  ...
  Future<void> _reconcile() async {
    ...
    for (final f in all.reversed) {
      ...
      if (total + size > maxBytes) { try { await f.delete(); } on Object {} } else { total += size; }
    }
  }
}
